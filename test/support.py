...
class DefaultExtensionTemplate(object):
    ...
    """
    const HPyDef_METH = hpy.helpers.Def_METH;
    const HPyDef_SLOT = hpy.helpers.Def_SLOT;
    const HPyDef_MEMBER = hpy.helpers.Def_MEMBER;
    const HPyDef_GETSET = hpy.helpers.Def_GETSET;
    const MODINIT = hpy.helpers.MODINIT;
    var mod_ctx: ?*hpy.HPyContext = null;
    var moduledefs = [_:null]?*hpy.HPyDef{
        %(defines)s
    };
    %(globals_defs)s
    var moduledef = hpy.helpers.ModuleDef{
    ...
    comptime {
        MODINIT(&mod_ctx, "%(name)s", &moduledef);
    }
    """)

    INIT_TEMPLATE_WITH_TYPES_INIT = textwrap.dedent(
        """
        hpy.helpers.Def_SLOT(&mod_ctx, generated_init_impl, hpy.HPy_mod_exec);
        fn generated_init_impl(ctx: ?*hpy.HPyContext, m: hpy.HPy) c_int {
            // Shouldn't really happen, but just to silence the unused label warning
            if (hpy.HPy_IsNull(m)) {
                return -1;
            }
    ...
    def expand(self):
        ...
        self.globals_table = []
        self.type_table = []
        self.output = ['const hpy = @import("./hpy-zig/src/hpy.zig");']
    ...
    def EXPORT_TYPE(self, name, spec):
        src = """
            if (!hpy.HPyHelpers_AddType(ctx, m, {name}, &{spec}, null)) {{
                return -1;
            }}
            """
        src = reindent(src, 4)
    ...
    def HPy_MODINIT(self, mod):
        return "hpy.helpers.HPy_MODINIT(&mod_ctx, {}, &{})".format(self.name, mod)
...
class ExtensionCompiler:
    ...
    def _expand(self, ExtensionTemplate, name, template):
        ...
        filename = self.tmpdir.join(name + '.zig')
        dump_file = self.dump_dir.joinpath(name + '.zig') if self.dump_dir else None
        if PY2:
    ...
    def compile_module(self, main_src, ExtensionTemplate=None, name='mytest', extra_sources=()):
        ...
        if hpy_abi == 'debug' or hpy_abi == 'trace':
            ...
            hpy_abi = 'hybrid'
        so_filename = zig_compile(str(self.tmpdir), ext,
...
def zig_compile(tmpdir, ext, hpy_devel, hpy_abi, compiler_verbose=0, debug=None):
    """Compile a Zig extension module using distutils."""
...
def _build(tmpdir, ext, hpy_devel, hpy_abi, compiler_verbose=0, debug=None):
    ...
    #
    dist = Distribution()
    dist.cmdclass['build_ext'] = BuildZigExt
    ...
    config_file = Path(str(tmpdir)) / 'hpy-zig' / 'src' / 'config.zig'
    with open(config_file, "w") as f:
        if hpy_abi == "universal":
            f.write('pub const hpy_abi = "HPY_ABI_UNIVERSAL";')
        elif hpy_abi == "hybrid":
            f.write('pub const hpy_abi = "HPY_ABI_HYBRID";')
        else:
            f.write('pub const hpy_abi = "HPY_ABI_CPYTHON";')
    ...
    try:
        ...
        if dist.hpy_ext_modules:
            if dist.hpy_ext_modules[0].sources:
                with open(dist.hpy_ext_modules[0].sources[0]) as src:
                    sys.stderr.write("------ Zig source file used -----\n\n")
                    for idx, line in enumerate(src):
                        sys.stderr.write(f"{idx+1} {line}")
        raise e
    finally:
        os.chdir(old_dir)
