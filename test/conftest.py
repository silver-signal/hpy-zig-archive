...
def compiler(request, tmpdir, hpy_devel, hpy_abi, ExtensionTemplate):
    ...
    # Move hpy-zig files to tmpdir
    hpy_zig_src_dir = Path(__file__).parent.parent / 'src'
    hpy_zig_src_list = list(hpy_zig_src_dir.rglob('*.zig'))
    hpy_zig_src_list.extend(list(hpy_zig_src_dir.rglob('*.c')))
    hpy_zig_src_list.extend(list(hpy_zig_src_dir.rglob('*.h')))
    
    dest_dir = Path(str(tmpdir)) / 'hpy-zig' / 'src'
    dest_dir.mkdir(parents=True, exist_ok=True)
    
    for file in hpy_zig_src_list:
