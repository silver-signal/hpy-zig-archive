...
var do_nothing = hpy.def.Method(&mod_ctx, "do_nothing", do_nothing_impl, hpy.HPyFunc_NOARGS);
pub fn do_nothing_impl(ctx: ?*hpy.HPyContext, self: hpy.HPy) callconv(.C) hpy.HPy {
    _ = self;
    return hpy.HPy_Dup(ctx, ctx.?.*.h_None);
}

var double_obj = hpy.def.Method(&mod_ctx, "double", double_obj_impl, hpy.HPyFunc_O);
pub fn double_obj_impl(ctx: ?*hpy.HPyContext, self: hpy.HPy, obj: hpy.HPy) callconv(.C) hpy.HPy {
    _ = @TypeOf(self);
    return hpy.HPy_Add(ctx, obj, obj);
}

var add_ints = hpy.def.Method(&mod_ctx, "add_ints", add_ints_impl, hpy.HPyFunc_VARARGS);
...
var add_ints_kw = hpy.def.Method(&mod_ctx, "add_ints_kw", add_ints_kw_impl, hpy.HPyFunc_KEYWORDS);
...
//pub const PointObject_AsStruct = hpy.def.Type_HELPERS(PointObject, hpy.HPyType_BuiltinShape_Object);
...
var Point_new = hpy.def.Slot(&mod_ctx, Point_new_impl, hpy.HPy_tp_new);
pub fn Point_new_impl(ctx: ?*hpy.HPyContext, cls: hpy.HPy, args: *const hpy.HPy, nargs: hpy.HPy_ssize_t, kwnames: hpy.HPy) callconv(.C) hpy.HPy {
    ...
    const h_point: hpy.HPy = hpy._HPy_New(ctx, cls, @as([*c]?*anyopaque, @ptrCast(@alignCast(&point))));
...
var Point_repr = hpy.def.Slot(&mod_ctx, Point_repr_impl, hpy.HPy_tp_repr);
pub fn Point_repr_impl(ctx: ?*hpy.HPyContext, self: hpy.HPy) callconv(.C) hpy.HPy {
    const point: [*c]PointObject = PointObject_AsStruct(ctx, self);
...
var mod_exec = hpy.def.Slot(&mod_ctx, mod_exec_impl, hpy.HPy_mod_exec);
pub fn mod_exec_impl(ctx: [*c]hpy.HPyContext, m: hpy.HPy) callconv(.C) c_int {
    const h_point_type: hpy.HPy = hpy.HPyType_FromSpec(ctx, &point_type_spec, null);
...
var pofdef = hpy.def.Module{
    .doc = "HPy Proof of Concept",
    .defines = &module_defines,
};

comptime {
    hpy.ModuleInit(&mod_ctx, "pof", &pofdef);
}
